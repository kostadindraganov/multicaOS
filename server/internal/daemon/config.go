...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	// Omnigent multi-harness orchestrator, driven over its local HTTP+SSE
	// server API. MULTICA_OMNIGENT_MODEL names a default Omnigent agent
	// (e.g. "polly" or "claude-native-ui").
	if e, ok := probe("MULTICA_OMNIGENT_PATH", "omnigent", "MULTICA_OMNIGENT_MODEL"); ok {
		agents["omnigent"] = e
	}
	if len(agents) == 0 && !overrides.AllowNoAgents {
		return Config{}, fmt.Errorf("no agent CLI found: install claude, codebuddy, codex, copilot, opencode, deveco, openclaw, hermes, pi, cursor-agent, kimi, kiro-cli, agy, qodercli, traecli, grok, qwen, or omnigent and ensure it is on PATH")
	}
...
var defaultAgentCommandNames = []string{
	...
	"pi", "cursor-agent", "copilot", "kimi", "kiro-cli", "codebuddy", "agy", "traecli", "grok", "qwen", "omnigent",
}
